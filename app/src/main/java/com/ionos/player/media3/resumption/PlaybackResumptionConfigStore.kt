...
class PlaybackResumptionConfigStore @Inject constructor(
    ...
    private fun PlaybackFileType(value: String): PlaybackFileType? {
        return PlaybackFileType.entries.firstOrNull { it.value == value }
    }

    private fun SearchType(name: String): SearchType? {
        return SearchType.entries.firstOrNull { it.name == name }
    }
}
