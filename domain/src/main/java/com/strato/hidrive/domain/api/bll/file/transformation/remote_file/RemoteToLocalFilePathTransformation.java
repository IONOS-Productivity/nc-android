package com.strato.hidrive.domain.api.bll.file.transformation.remote_file;

import com.strato.hidrive.domain.interfaces.actions.Transformation;
import com.strato.hidrive.domain.provider.HidrivePathProvider;
import com.strato.hidrive.domain.utils.FileUtils;

import javax.inject.Inject;

/**
 * Created by yaz on 8/2/16.
 */
public class RemoteToLocalFilePathTransformation implements Transformation<String, String> {
	private final HidrivePathProvider pathProvider;

	@Inject
	public RemoteToLocalFilePathTransformation(HidrivePathProvider pathProvider) {
		this.pathProvider = pathProvider;
	}

	@Override
	public String transform(String remotePath) {
		String currentUserFolderPath = this.pathProvider.getCurrentUserFolderPath();
		String localPath = remotePath.equals(currentUserFolderPath)
				? FileUtils.ROOT_PATH
				: remotePath.replaceFirst(currentUserFolderPath, "");
		if (!localPath.startsWith(FileUtils.ROOT_PATH)) {
			localPath = FileUtils.ROOT_PATH + localPath;
		}
		return localPath;
	}
}
